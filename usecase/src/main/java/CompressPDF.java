...
public class CompressPDF {
     ...
     * @return
     */
    public static byte[] reduce(byte[] b, Long peso) {
        byte[] pdfByte = b;
    ...
    private static byte[] executeReduce(byte[] b) {
        byte[] pdfByte = null;
    ...
    private static void getImageLocationAndSize(PDDocument document, PDPage page) throws IOException {
    ...
    private static void validateImage(PDDocument document, PDPage page, BufferedImage image, Metadata metadata) throws IOException {
    ...
    private static Dimension fixVertical(Metadata metadata) {
    ...
    private static Dimension fixHorizontal(Metadata metadata) {
        Dimension dimension = new Dimension();
    ...
    private static String getOrientation(PDPage page) {
        String orientation;
    ...
    private static void resizeImage(PDDocument document, PDResources pdResources, BufferedImage originalImage, Dimension dimension, COSName cname) throws IOException {
    ...
    private static boolean isLessEqualToWeight(byte[] b, long peso) {
        return b.length <= peso * 1024L;
    }

    private static byte[] getBytesFromFile(PDDocument document) throws IOException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        document.save(baos);
        return baos.toByteArray();
    }

    private static boolean isValid(byte[] original, byte[] file) {
